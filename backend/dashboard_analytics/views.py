...
class SalesChartAPIView(APIView):
    ...
    def get(self, request):
        ...
        base_curves = {
            '2026': [18.5, 24.2, 31.0, 28.4, 39.5, 46.2, 54.1, 58.0, 48.5, 42.0, 53.5, 64.0],
            '2025': [15.2, 22.4, 28.1, 24.5, 35.8, 42.0, 48.6, 52.3, 44.1, 38.5, 49.2, 58.0],
            '2024': [12.0, 18.5, 24.0, 21.2, 30.5, 36.8, 41.2, 45.0, 39.1, 34.0, 42.8, 51.5],
            '2023': [10.5, 14.2, 19.8, 17.5, 25.0, 30.1, 35.4, 38.0, 32.5, 28.0, 35.2, 43.0],
        }

        curve = base_curves.get(year, base_curves['2026'])
        real_tx_total = sum([float(t.amount) for t in Transaction.objects.all()])
        if real_tx_total > 0 and year == '2026':
